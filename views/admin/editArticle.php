            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="javascript:submit();" method="POST" style="text-align: center;">
                        <input class="title" type="text" name="title" placeholder="Title" id="title" required value="<?= $article->title?>">
                        <input type="text" style="display: none;" id="articleId" value="<?= $article->_id?>"
                        <br>
                        <div class="dropdown  subject-input">
                            <input type="text" id="inputSubject" name="subject" value="<?= $currSubject->name ?>"
                            placeholder="Choose subject that this article belong to" onkeypress="return false;" required>
                            <input style="display:none" type="text" id="inputSubjectId" value="<?= $currSubject->_id ?>" required>
                            <div class="dropdown-content dropdown-bg">
                                <?php
                                foreach ($subjects as $subject) {
                                    ?> <div class='checkbox-tags'><input class='subject' type='radio' name='subject' id='" . $subject->_id . "' value='" . $subject->_id . "'
                                     onclick='check()' <?= ($currSubject->_id == $subject->_id) ? "checked" : ""; ?> />
                                     <?= "<label style='margin-left: 10px' for='" . $subject->_id . "'>" . $subject->name . "</label></div><br>";
                                }
                                ?>
                            </div>
                        </div>
                        <Textarea class="content" rows="50" name="content" placeholder="Your post's content." id="content" required> <?= $article->html?></Textarea>
                        <br>
...
<script>
    ...
    function submit() {
        ...
        $inputSubjectId = $inputSubjectId ? $inputSubjectId : "<?= $currSubject->_id ?>";
